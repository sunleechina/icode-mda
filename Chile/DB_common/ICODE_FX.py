...
# Final functions
#-----------------------------------------------------------------------
def AIS_upload_DB(filename, formdate = '%Y-%m-%d %H:%M:%S.0', dbinfo = ['localhost','root','root','vessels','vessel_history'], coi = np.array(['mmsi','recvtime','latitude','longitude','sog','rot','rxstnid'])):
	"""
	...
	Parametros:
		...
		- dbinfo: Informacion de la base de datos de destino de la forma
		  [host, usuario, contrasena, nombre de la BD, tabla].          (type:string)
		- coi: Columnas de interes a subir a la BD.                     (type:string)
	"""
	DB.dbConnection(dbinfo[0],dbinfo[1],dbinfo[2],dbinfo[3])
	if DB.checkTableExists(dbinfo[4]):
		if os.path.isfile(filename):
			data = RP.AIS2list(filename, formdate, coi)
			DB.uploadToDB(dbinfo[4], data)
		else:
			print "*** ERROR: File does't exists ***"
	else:
		print "*** ERROR: "+dbinfo[4]+" doesn't exists ***"
	DB.dbDisconnect()

#-----------------------------------------------------------------------
def AIS_date_hist(sdy, ndy = 0, bins = 1000, plim = [-90, 90,-180, 180], formdate = '%Y-%m-%d', dbinfo = ['localhost','root','root','vessels','vessel_history'], folder = 'histograms'):
	"""
	...
	Parametros:
		...
		- plim: Limites geograficos [Latmin, Latmax, Lonmin, Lonmax].   (type:float)
		- formdate: Formato de fecha.                                   (type:string)
		- dbinfo: Informacion de la base de datos de destino de la forma
		  [host, usuario, contrasena, nombre de la BD, tabla].          (type:string)
	...
	"""
	con = DB.dbConnection(dbinfo[0],dbinfo[1],dbinfo[2],dbinfo[3])
	if DB.checkTableExists(dbinfo[4]):
		...
		for n in range(ndy+1):
			...
			query += geo_query
			data = np.array(DB.readfromDB(dbinfo[4], 'MMSI, Latitude, Longitude', query), dtype = float)
			#For the histogram
			if data != []:
				datacount += 1
				#Make the histogram
				...
				xfile.close()
		if datacount == 0:
			print "*** ERROR: there's no data for the selected days ***"
	else:
		print "*** ERROR: "+dbinfo[4]+" doesn't exists ***"
	con = DB.dbDisconnect()
